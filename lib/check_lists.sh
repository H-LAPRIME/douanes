...
WHITELIST="${DOUANES_WHITELIST:-/etc/douanes/whitelist.conf}"
BLACKLIST="${DOUANES_BLACKLIST:-/etc/douanes/blacklist.conf}"

if [[ ! -f "$WHITELIST" && -f "$PROJECT_DIR/conf/whitelist.conf" ]]; then
    WHITELIST="$PROJECT_DIR/conf/whitelist.conf"
fi

if [[ ! -f "$BLACKLIST" && -f "$PROJECT_DIR/conf/blacklist.conf" ]]; then
    BLACKLIST="$PROJECT_DIR/conf/blacklist.conf"
fi
